...
describe('mainMenuAction', () => {
	it('exports a TabIds enum', () => {
		expect(Object.keys(TabIds).length).toBe(8);
		...
		expect(TabIds.EXTENSION).toBe('extension');
		...
		expect(TabIds.valueOf(6)).toEqual(TabIds.EXTENSION);
		expect(TabIds.valueOf(7)).toBeNull();
	});
});
