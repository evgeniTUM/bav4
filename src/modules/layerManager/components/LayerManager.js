...
export class LayerManager extends MvuElement {
	...
	_getButtons(model) {
		...
		const deactivateAll = () => {
			draggableItems.filter((i) => !i.isPlaceholder).forEach((layer) => modifyLayer(layer.id, { visible: false }));
		};
		...
		return draggableItems.filter((i) => !i.isPlaceholder).length > 0
			? html`<div class="layermanager__actions">
					...
					></ba-button>
					<ba-button
						id="button_disable_all"
						.label=${translate('layerManager_deactivate_all')}
						.type=${'secondary'}
						@click=${deactivateAll}
						style="border-right: 1px dotted var(--header-background-color);"
					></ba-button>
