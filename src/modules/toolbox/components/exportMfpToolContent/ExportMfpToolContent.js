...
export class ExportMfpToolContent extends AbstractToolContent {
	...
	onInitialize() {
		...
		this.observe(
			...
			(data) => this.signal(Update_PrintLegend, data)
		);
		this.observe(
			(state) => state.media,
				...
				<div class="tool-section separator" style="margin-top:1em">
					...
					<div class="tool-section" style="margin-top:1em">
						<ba-checkbox
							id="print-legend"
							.checked=${printLegend}
							.title=${translate('toolbox_exportMfp_print_legend')}
							@toggle=${onChangePrintLegend}
							><span>${translate('toolbox_exportMfp_print_legend')}</span></ba-checkbox
						>
					</div>
				</div>
			</div>`;
